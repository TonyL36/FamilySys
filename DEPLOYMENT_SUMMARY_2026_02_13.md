...
**修复状态 (2026-02-14)**：
已修改逻辑，优先使用工作目录 (`/opt/familysys/family.db`) 的数据库文件。若不存在，则从 JAR 包提取一份到该目录。此修改确保了重启服务后数据不会丢失。
...
**修复状态 (2026-02-14)**：
已将所有前端文件中的 `localhost` / `127.0.0.1` 批量替换为服务器公网 IP `47.109.193.180`。
...
### 5.2 关键修复
1.  **数据持久化**：`DatabaseConnection.java` 逻辑已更新，不再使用临时文件。
2.  **前端连接**：修复了 `Failed to fetch` 问题，前端代码已正确指向服务器 IP。
3.  **自动化部署**：`deploy_update.ps1` 脚本已更新，支持一键部署到新目录结构。
