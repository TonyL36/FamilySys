...
3. 注意系统数据库关系数据产生应顺序：父母 → 配偶 → 子女（长/次/小）。
4. 数据来源：只取“最小基础关系”父子和母子从前端录入模拟用户，让系统自动生成更复杂关系。
...
九、下午3点后编程内容技术文档
1. 前端展示调整
   - 远亲结果中隐藏 result-summary 的称谓与描述，仅在失败/异常时显示提示。
   - 关系链路上的边文字恢复显示，保证链路可读性。
2. 血缘紧密度计算调整
   - 使用 φ = (1/2)^(血缘步数 - 1)。
   - 祖孙/外祖辈类关系按 2 步计算，提高代际跨度的权重。
3. 前端远端版本同步与清理
   - family-frontend 与 family-frontend-remote 的远亲页面逻辑保持一致。
   - 移除未使用的样式定义，避免冗余代码。
4. 快速自检要点
   - 结果成功时不显示 result-summary。
   - 关系链路线条上有称谓文字。
   - 远端页面渲染与本地一致，仅 API 地址不同。
